...
fn paper_close_when_spread_converges() {
    ...
    }
    {
        ...
        pb.update_asks(&[Level {
            price: Fixed64::from_float(50020.0),
            qty: 1_000_000,
        }]);
    }
    ...
    // Now move market: perp collapses below spot → spread inverted → exit
    {
        let mut pb = engine.perp_book.write();
        pb.update_bids(&[Level {
            price: Fixed64::from_float(49990.0),
            qty: 1_000_000,
        }]);
        pb.update_asks(&[Level {
            price: Fixed64::from_float(49995.0),
            qty: 1_000_000,
        }]); // perp_ask < spot_bid
    }
    ...
    assert!(
        ...
        "should be closed after spread inversion"
    );
}
...
fn exit_spread_raw_used_for_close() {
    ...
    // Spread narrows to 4 USDT (≤ 5 → should close)
    {
        let mut pb = engine.perp_book.write();
        pb.update_asks(&[Level {
            price: Fixed64::from_float(50004.0),
            qty: 1_000_000,
        }]);
    }
    strat.on_tick();
    assert!(
        !strat.is_position_open(),
        "should close when spread ≤ exit_spread_raw"
    );
}
...
fn min_profit_guard_blocks_graceful_close_at_zero() {
    ...
    assert!(
        strat.is_position_open(),
        "min_profit should block close when spread just reached 0"
    );
}
